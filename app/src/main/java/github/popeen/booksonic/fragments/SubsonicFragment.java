 ...
 */
package github.popeen.booksonic.fragments;
...
import github.popeen.booksonic.activity.SubsonicFragmentActivity;
import github.popeen.booksonic.adapter.SectionAdapter;
import github.popeen.booksonic.domain.MusicDirectory;
import github.popeen.booksonic.domain.Playlist;
import github.popeen.booksonic.service.MusicService;
import github.popeen.booksonic.service.MusicServiceFactory;
import github.popeen.booksonic.service.ServerTooOldException;
...
import github.popeen.booksonic.view.GridSpacingDecoration;
import github.popeen.booksonic.view.PlaylistSongView;
import github.popeen.booksonic.dsub.R;
import github.popeen.booksonic.activity.SubsonicActivity;
import github.popeen.booksonic.domain.Artist;
...
import github.popeen.booksonic.service.DownloadFile;
import github.popeen.booksonic.service.DownloadService;
import github.popeen.booksonic.service.MediaStoreService;
import github.popeen.booksonic.service.OfflineException;
import github.popeen.booksonic.util.MenuUtil;
import github.popeen.booksonic.util.LoadingTask;
import github.popeen.booksonic.util.UserUtil;
...
public class SubsonicFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
	...
	public void toggleStarred(MusicDirectory.Entry entry) {
		toggleStarred(entry, null);
	}

	public void toggleStarred(final MusicDirectory.Entry entry, final OnStarChange onStarChange) {
		...
		new SilentBackgroundTask<Void>(context) {
			...
			protected Void doInBackground() throws Throwable {
				...
				if(Util.isTagBrowsing(context) && !Util.isOffline(context)) {
					musicService.setStarred(null, Arrays.asList(new MusicDirectory.Entry(entry)), null, starred, null, context);
				} else {
					musicService.setStarred(Arrays.asList(new MusicDirectory.Entry(entry)), null, null, starred, null, context);
				}
				return null;
			}
	...
	protected void downloadRecursively(final List<MusicDirectory.Entry> albums, final boolean shuffle, final boolean append) {
		new RecursiveLoader(context) {
			@Override
			protected Boolean doInBackground() throws Throwable {
				musicService = MusicServiceFactory.getMusicService(context);
	...
	protected void setRating(MusicDirectory.Entry entry) {
		setRating(entry, null);
	}
	protected void setRating(final MusicDirectory.Entry entry, final OnRatingChange onRatingChange) {
	...
	protected void setRating(MusicDirectory.Entry entry, int rating) {
		setRating(entry, rating, null);
	}
	protected void setRating(final MusicDirectory.Entry entry, final int rating, final OnRatingChange onRatingChange) {
	...
	protected abstract class EntryInstanceUpdater {
		...
		public abstract void update(MusicDirectory.Entry found);
